...
2. Buka **phpMyAdmin**, pilih database yang baru dibuat, buka tab **Import**, lalu upload file `database/schema.sql` yang ada di paket ini. Klik **Go**.
3. Setelah selesai, pastikan muncul 10 tabel: `users`, `customers`, `suppliers`, `work_orders`, `pr_items`, `seal_items`, `transport_items`, `master_products`, `master_buyers`, `activity_log`.
